...
<!-- Nice intro; concise, but lays a good general background. As an aside, if this is a topic you are interested in, I can connect you with UO faculty who are looking at these same issues. -->
# Methods 
...
<!-- If ypossible, it would be nice to include a copy of the survey in an Appendix. See https://crsh.github.io/papaja_man/writing.html#appendices -->
## Data Import
  ...
  The four quantitative variables of the organized data set represented numerical counts of specific types of disciplinary actions taken by the school.  The four types of disciplinary actions included: disciplinary actions recorded, incidents recorded, incidents reported to police, and out-of-school suspensions.<!-- I know these are separate variables, but do they represent distinct categories? For example, are "incidents recorded" totally separate from "incidents reported to police"? It may be, but if not, summing them would not be appropriate (this may explain the excessive total incidents seen in your figures). -->  Like the other variable types, the names of these variables were renamed using the `rename()` function in R Markdown.  To make the analysis of these totals more efficient, these variables were combined into a single variable using the `mutate()` function to represent the total number of disciplinary actions per school.  
...
# It would be nice if the x-axis and the facets were ordered.
...
tidy_new <- tidy_disc1 %>% 
  mutate(minority_percentage = factor(minority_percentage,
                           levels = c("5 to 20 percent",
                                      ...
                                      "50 percent or more",
                                      "Less than 5 percent")))
# Order the x-axis (see below). I took one point off for these.
ggplot(tidy_new, aes(fct_relevel(minority_percentage, "Less than 5 percent", "5 to 20 percent"), total_discipline_actions)) +
  geom_col(position = "dodge", fill = "blue") +
  theme_minimal() +
  scale_fill_discrete(name = "Discipline Type", labels = c("Disciplinary Actions Recorded", 
                                                           "Incidents Recorded",
...
```
<!-- In the code chunk options here, you need results = "asis" to show the table correctly. I took two point for this. I also took a point because the tables ran off the pages. -->
```{r, descriptives tidydisc1, include=TRUE, results = "asis"}
...
#These figures are a bit hard to interpret. 
...
tidy_table2 <- tidy_disc2 %>% 
  ...
  summarize(sum(total_discipline_actions))
```
<!-- Again, you need results = "asis" in the code chunk options.  -->
```{r descriptives tidydisc2, include=TRUE, results = "asis"}
...
```{r model tables, include=TRUE, results="asis"}
...
#The summary below was showing in your final document. I took one point for that.
...
ggplot(safe_organized) +
  ...
  labs(x = "School Urbanicity by School Level", 
       ...
       tag = "(6)",
       fill = "School level")
# Order your factors so the order is intuitive.
ggplot(safe_organized) +
  geom_bar(aes(urbanicity, fill = level),
           position = "dodge") +
  facet_wrap(~enrollment, nrow = 1) +
...
ggplot(safe_organized, aes(out_of_school_suspensions, disciplinary_actions_recorded)) +
  ...
  geom_smooth() +
  theme_minimal() +
#  scale_x_log10() + #Try this scale and see what you think. Sometimes taking the log of the x or y axis helps show the relation.
...
## Tidy data
<!-- This section would have been perfect for inline code, but I didn't see any so I had to take points.   -->
<!-- Also, full disclosure, I did not review the text to see if it matched the tables. -->
...
<!-- You don't want results="asis" below, but probably eval=FALSE   -->
```{r descriptive counts, eval=FALSE}  

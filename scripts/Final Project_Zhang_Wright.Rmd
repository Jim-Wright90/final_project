...
abstract: The purpose of this paper is to explore data from the “2005-2006 School Survey on Crime and Safety” using functions in R Markdown.  The data file with results from the survey was acquired from data.gov and consists of 525 columns by 2,724 rows.  To complete this project, 14 total variables were selected from the data set to transform from its messy state to tidy data.  The 14 variables included the school ID variable, four categorical variables, five logical variables, and four quantitative variables.  Linear models were completed to determine which categorical variables most highly correlated with total disciplinary actions within the school and to determine the correlation between the provision of teacher training and out of school suspensions (OSS).  Results of the analyses found that the strongest predictors of disciplinary actions taken by schools are schools that were coded as either a high school or combined middle school and high school located in an urban city with a minority percentage of 20% or more.  It was also determined that the provision of teacher training occurred in a majority of schools; however, these trainings did not have a significant impact on reducing the number of out of school suspensions. 
...
## Teacher Training Relationship to OSS
  ...
  Plots 9 and 10 display descriptive counts on the number of schools that provide teacher training on classroom manageemnt and positive behaviorl intervention, respectively.  Both plots are organized by school urbanicity and faceted by school enrollment.  The purpose of Plot 11 was to display the relationship of providing specific teacher trainings and how this may predict OSS.  Overall, Plot 11 and the results of Model 4 provide preliminary evidence that the provision of teacher training can be a useful method for schools to implement to reduce OSS; however, further analysis is warranted to determine a more accurate effect on the provision of teacher training on reducing OSS. 
...
```{r descriptive counts, include = FALSE}  
